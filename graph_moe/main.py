...
for epoch in range(args.epoch):  # 进行训练循环，共200个epoch
    ...
    with torch.no_grad():
        ...
        loss_epoch = F.binary_cross_entropy(torch.tensor(logistic_pred), torch.tensor(y_true))
...
with open("./model/record.txt", "a") as f:
    f.write(f"traindata {args.train_datasets}, testdata {args.test_datasets}, num_experts {args.expert_num}, train_epoch {args.epoch}, batch {args.batch}, inv {args.inv_fuc}, lr {args.lr}, lambda {args.lambda_inv}, Loss {loss_mean}, accuracy {accuracy}, loss_eval {loss_epoch}, accuracy_eval {accuracy_epoch}, f1_score {f1score_eval}\n")