...
class MoeTemporalBlock(nn.Module):
    def __init__(self, hidden_size: int, intermediate_size: int, out_channels: int):
        super().__init__()
        ...
        # self.down_proj = nn.Linear(self.intermediate_size, self.out_channels, bias=False)
        self.lin = Linear(self.hidden_size , self.out_channels, bias=False,
                          weight_initializer='glorot')

    def forward(self, hidden_state):
        # return self.down_proj(F.relu(self.gate_proj(hidden_state)) * self.up_proj(hidden_state))
        return F.relu(self.lin(hidden_state))
...
class GCNMoeSparseExpertsLayer(nn.Module):
    def __init__(self, in_channels: int, out_channels: int):
        ...
        self.out_channels = out_channels
        self.num_experts = args.expert_num
        self.norm_topk_prob = False

        moe_intermediate_size = self.args.inter_dim // self.top_k
        ...
        self.shared_expert = MoeTemporalBlock(
            hidden_size=self.hidden_size,
    ...
    def forward(self, hidden_states: torch.Tensor):
        ...
        final_hidden_states = torch.zeros(
            (sequence_length, self.out_channels), dtype=hidden_states.dtype, device=hidden_states.device
        )
...
class My_GCNConv(MessagePassing):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.lin_moe = GCNMoeSparseExpertsLayer(self.in_channels, self.out_channels)
    ...
    def forward(self, x: Tensor, edge_index: Adj,
        ...
        out,router_logits = self.lin_moe(x)
