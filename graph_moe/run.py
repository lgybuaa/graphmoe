...
from data_utils import get_datasets, get_dataloader
...
# 加载数据
train_datasets, test_datasets = get_datasets(args.train_datasets, args.test_datasets)
len_train_datasets = len(train_datasets)
len_test_datasets = len(test_datasets)
train_loader = get_dataloader(train_datasets, args.batch, args.workers, args.neg_sampling_ratio)
test_loader = get_dataloader(test_datasets, args.batch, args.workers, args.neg_sampling_ratio)
...
# 训练模型
metrics_dict = train_epoch(model, train_loader, test_loader, optimizer, args.devices[0], args.epoch, len_train_datasets, len_test_datasets)

# best eval acc
best_eval_acc = max(metrics_dict['accuracy_eval'])
best_epoch  = metrics_dict['accuracy_eval'].index(best_eval_acc)
...
with open("./model/record.txt", "a") as f:
    f.write(f"traindata {args.train_datasets}, testdata {args.test_datasets}, num_experts {args.expert_num}, topk {args.topk}, train_epoch {args.epoch}, batch {args.batch}, inv {args.inv_fuc}, lr {args.lr}, lambda {args.lambda_inv}, Loss {metrics_dict['loss_train'][-1]}, accuracy {metrics_dict['accuracy_train'][-1]}, loss_eval {metrics_dict['loss_eval'][-1]}, accuracy_eval {metrics_dict['accuracy_eval'][-1]}, f1_score {metrics_dict['f1score_eval'][-1]} best_eval_acc {best_eval_acc} best_epoch {best_epoch}\n")